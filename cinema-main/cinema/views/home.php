...
$homeMovies = array_slice($homeMovies, 0, 15);
...
if ($tab === 'soon') {
    ...
    foreach (array_chunk($homeMovies, 3) as $items) {
        $chunkFirstDate = null;
        foreach ($items as $item) {
            $candidate = trim((string) ($item['first_show_date'] ?? $item['release_date'] ?? ''));
            if ($candidate === '') {
                continue;
            }
            if ($chunkFirstDate === null || $candidate < $chunkFirstDate) {
                $chunkFirstDate = $candidate;
            }
        }
        $rows[] = ['date' => ($chunkFirstDate ?: $kyivToday), 'items' => $items];
    }
}
?>
